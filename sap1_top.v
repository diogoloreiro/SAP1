...
module sap1_top (
    ...
    output wire       hlt,
    output wire [7:0] bus_w,    // barramento W exposto (observacao/debug)
    output wire [5:0] tstate,   // contador de anel T1..T6 (debug)
    output wire [3:0] opcode,   // opcode atual no IR (nibble alto) - p/ display
    output wire [7:0] acc,      // acumulador A (registrador real) - p/ display
    output wire [7:0] breg      // registrador B (registrador real) - p/ debug
);
    // ---- barramento W ----
    wire [7:0] w_bus;
    ...
    // ---- saidas de observacao (debug na placa) ----
    assign bus_w  = w_bus;
    assign tstate = ring;
    assign opcode = ir_opcode;   // expoe o opcode do IR para o display
    assign acc    = acc_out;     // valor real do acumulador A
    assign breg   = b_out;       // valor real do registrador B
